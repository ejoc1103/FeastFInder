<template>
    <div class="groups-container">
        ...
        <button v-if="$store.state.showVoteView" v-on:click="hideGroup()" class="view-group">
            Back To Feasts View
        </button>

        <div v-if="!$store.state.showGroupForm && !feastOpen" class="groups">
            <div v-for="group in filteredGroups" :key="group.id" :class="isActive(group.is_active)" class="group">
                <div v-if="!$store.state.showVoteView">
                    <h2>{{ group.vote_name }}</h2>
                    <button v-on:click="goToVoting(group.vote_id)" class="view-group">
                        Vote Now
                    </button>
                    <button v-on:click="showGroup(group.vote_id)" class="view-group">
                        View Feast
                    </button>
        ...
        <div v-if="feastOpen" class="voting">
            <GroupCard :group="groups[group]" />
        </div>
    </div>
</template>
...
.groups {
    ...
    flex-wrap: wrap;
    justify-content: center;
    /* Centers the group cards horizontally */
    align-items: center;
    /* Centers the group cards vertically */
    gap: 20px;
    width: 100%;
}
...
.group {
    ...
    min-width: 250px;
    /* Minimum width to avoid cards becoming too small */
    max-width: 300px;
    /* Maximum width to control excessive card size */
    padding: 15px;
    border-radius: 10px;
    box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);
    text-align: center;
    /* Ensure all text inside the card is centered */
}
...
.show-all-groups {
    ...
    color: #E6E6FA;
    width: 100%;
    /* Ensures the button takes full width of its container */
    max-width: 180px;
    /* Limits button size */
    margin-top: 10px;
    transition: background-color 0.3s, transform 0.3s;
    text-align: center;
    /* Ensure text on the button is centered */
}
...
.voting {
    ...
    flex-direction: column;
    align-items: center;
    /* Centers content inside the voting div */
